(function () {
    window.initSearchFilter = function (options) {
        const {
            inputId,
            recipeSearchUrl,
            extraVisibilityMatch = () => true,
        } = options;
        ...
        async function fetchSuggestions(q) {
            ...
            const [recipes, tags, ingredients] = await Promise.all([
                fetch(recipeSearchUrl + '?search=' + enc).then(r => r.json()),
        ...
        function filterCards() {
            ...
            cards.forEach(card => {
                ...
                const visible = extraVisibilityMatch(card)
                    && tagFilters.every(t => cardTags.includes(t))
                    && ingFilters.every(i => cardIngs.includes(i));
